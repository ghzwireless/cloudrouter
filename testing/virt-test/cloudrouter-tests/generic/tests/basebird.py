...
def run(test, params, env):
    ...
    cloudrouter = session.cmd("sudo dnf install -y bird",timeout=300)
    ...
    birdcmd = "sudo birdc -v show status|grep '0013 Daemon is up and running'"
    cloudrouter = session.cmd(birdcmd)
    logging.info("birdc cmd show daemon is running: %s", cloudrouter)
    cloudrouter = session.cmd("sudo systemctl disable bird")
    ...
    cloudrouter = session.cmd("sudo systemctl stop bird")

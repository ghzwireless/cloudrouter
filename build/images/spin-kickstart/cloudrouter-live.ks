...
mountPersistentHome() {
  ...
  if [ -d /home/cloudrouter ]; then USERADDARGS="-M" ; fi
}
...
action "Adding live user" useradd \$USERADDARGS -c "CloudRouter Live System User" cloudrouter
passwd -d cloudrouter > /dev/null
usermod -aG wheel cloudrouter > /dev/null
